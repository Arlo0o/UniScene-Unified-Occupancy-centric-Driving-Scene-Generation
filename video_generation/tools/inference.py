...
def main():
    ...
    dataloader_args = dict(
        dataset=dataset,
        batch_size=1, # cfg.get("batch_size", None),
    ...
    dataloader_args['batch_size'] = 1 #cfg.val.get("batch_size", 1)
    dataloader_args['num_workers'] = 0 # cfg.val.get("num_workers", 2)
    val_dataloader, val_sampler = prepare_dataloader(
        bucket_config=cfg.get("bucket_config", None),
        num_bucket_build_workers=cfg.get("num_bucket_build_workers", 1),
        **dataloader_args,
    )
    val_dataloader, val_sampler = dataloader, sampler
