...
from uniscenev2.acceleration.communications import all_to_all, split_forward_gather_backward
...
from uniscenev2.models.layers.rope import PositionGetter3D, RoPE3D
...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, frame: int = 8, height: int = 8, width: int = 8) -> torch.Tensor:
        B, N, C = x.shape
        ...
        if self.qk_norm_legacy:
            ...
            q, k = self.q_norm(q), self.k_norm(k)
            if self.rope:
                if isinstance(self.rotary_emb, RoPE3D):
                    pos_thw = self.position_getter(B, t=frame, h=height, w=width, device=q.device)
                    q = q.permute(0, 2, 1, 3)
                    k = k.permute(0, 2, 1, 3)
                    q = self.rotary_emb(q, pos_thw)
                    k = self.rotary_emb(k, pos_thw)
                    q = q.permute(0, 2, 1, 3)
                    k = k.permute(0, 2, 1, 3)
                else:
                    q = self.rotary_emb(q)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, cond: torch.Tensor = None, frame: int = 8, height: int = 8, width: int = 8) -> torch.Tensor:
        B, N, C = x.shape
        # flash attn is not memory efficient for small sequences, this is empirical
        enable_flash_attn = self.enable_flash_attn and (N > B)
