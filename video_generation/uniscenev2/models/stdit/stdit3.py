...
from uniscenev2.acceleration.communications import gather_forward_split_backward, split_forward_gather_backward
from uniscenev2.acceleration.parallel_states import get_sequence_parallel_group
...
from uniscenev2.utils.ckpt_utils import load_checkpoint
from uniscenev2.utils.misc import warn_once
from uniscenev2.models.stdit.embedder import RayMapControlTempEmbedding
from uniscenev2.models.utils import zero_module, load_module
...
class STDiT3(PreTrainedModel):
    ...
    def __init__(self, config: STDiT3Config):
        ...
        # if config.frame_emb_cls is not None:
        #     # init frame encoder
        ...
        # self.controlnet_cond_embedder = None
        # if config.map_embedder_cls is not None:
        #     # init map 2D encoder
        ...
        if config.raymap_embedder_cls is not None:
            ...
            self.controlnet_cond_embedder = load_module(config.raymap_embedder_cls)(
                conditioning_embedding_channels=self.hidden_size // 2,
        ...
        self.controlnet_cond_embedder_temp = RayMapControlTempEmbedding(
        ...
        if config.freeze_temporal_blocks:
            logging.info(f"freeze_temporal_blocks")
            ...
            if self.control_blocks_t is not None:
                logging.info(f"control_blocks_t")
        ...
        if config.only_train_base_blocks:
            # first freeze all
            logging.info(f"only_train_base_blocks")
        ...
        if config.freeze_old_params:
            logging.info(f"freeze_old_params")
            for param in self.parameters():
                param.requires_grad = False
    ...
    def encode_cond_sequence(self, cams, y, mask, drop_cond_mask, drop_frame_mask):  # changed
        b = len(y)
    ...
    def encode_ray_map(self, cams, rel_pose, NC, h_pad_size, x_shape):
        cam_K = cams[:,:,0,:,:3]
        ego_to_world = rel_pose[:,:,0]
        ...
        H, W = x_shape[-2:]
        plucker_embed = self.ray_condition(cam_K, c2w, H*8, W*8, device=cam_K.device)
        plucker_embed = rearrange(plucker_embed, "(B NC) C T ... -> B NC T C ...", NC=NC)
        B, T = plucker_embed.shape[0], plucker_embed.shape[2]
        controlnet_cond = self.controlnet_cond_embedder(plucker_embed)
    ...
    def forward(self, x, timestep, y, cams, rel_pos, fps,
        ...
        y, y_lens = self.encode_cond_sequence(cams, y, mask, drop_cond_mask, drop_frame_mask)  # (B, L, D)
        ...
        c = self.encode_ray_map(cams, rel_pos, NC, h_pad_size, x_in_shape)
...
@MODELS.register_module("STDIT3")
def MVSTDIT3(from_pretrained=None, force_huggingface=False, **kwargs):
...
if __name__ == "__main__":
    ...
    config = STDiT3Config(
        ...
        use_st_cross_attn=False,
        uncond_cam_in_dim=(3, 7),
        cam_encoder_cls="uniscenev2.models.stdit.embedder.CamEmbedder",
        ...
        raymap_embedder_cls="uniscenev2.models.stdit.embedder.RayMapControlEmbedding",
    ...
    model = STDiT3(config).to(device).to(torch.bfloat16)
    ...
    x = torch.rand(1,192,9,44,88).to(device,torch.bfloat16)#torch.rand(6, 16, 17, 28, 50).to(device,torch.bfloat16)
    timestep = torch.rand(1).to(device,torch.bfloat16)
    y = torch.rand(1, 1, 300, 4096).to(device,torch.bfloat16)
    cams = torch.rand(6, 33, 1, 3, 7).to(device,torch.bfloat16)
    rel_pos = torch.rand(6, 33, 1, 4, 4).to(device,torch.bfloat16)
    ...
    x_mask=torch.ones(1, 9).to(device,torch.bool)
    import time
    embed()
    exit()
    with torch.no_grad():
