...
class ImageAug3D:
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if  data.get("lidar") != None:
            lidars = data["lidar"]
            new_lidars = []
            for lidar in lidars:
                resize, resize_dims, crop, flip, rotate = self.sample_augmentation(data)
                
                # new_lidar1 = lidar.resize(resize_dims,  Image.NEAREST)
                # new_lidar = new_lidar1.crop(crop)
                
                lidar_array = np.array(lidar,  dtype=np.float32) 
                if 0:
                    # 获取原始图像的尺寸
                    original_height, original_width = lidar_array.shape
                    # 计算水平和垂直方向的缩放因子
                    scale_factor_x, scale_factor_y = 1// self.resize_lim[0], 1// self.resize_lim[1]
                    # 确保高度和宽度是scale_factor的整数倍
                    new_width , new_height= int(original_width // scale_factor_x * scale_factor_x), int(original_height // scale_factor_y * scale_factor_y)
                    # 裁剪图像以确保其尺寸是scale_factor的整数倍
                    lidar_array = lidar_array[:new_height, :new_width]
                    # 重塑数组以便应用最大池化
                    lidar_array = lidar_array.reshape(new_height // int(scale_factor_y), int(scale_factor_y), new_width // int(scale_factor_x), int(scale_factor_x))
                    # 应用最大池化
                    lidar_array = lidar_array.max(axis=(1, 3))
                    lidar_array = lidar_array[ crop[1]:crop[3], crop[0]:crop[2] ]
                if len(lidar_array.shape) == 2:  # 确认是否为二维数组
                    lidar_array = np.stack((lidar_array,) * 3, axis=-1)
                new_lidars.append(lidar_array)
            data["lidar"] = new_lidars
        
        if  data.get("depth_map") != None:
            depths = data["depth_map"]
            new_depths = []
            for depth in depths:
                resize, resize_dims, crop, flip, rotate = self.sample_augmentation(data)  ## 下采样与增强
                new_depth1 =  cv2.resize(depth, resize_dims, interpolation=cv2.INTER_NEAREST)  
                new_depth =  new_depth1[ crop[1]:crop[3], crop[0]:crop[2] ]
                depth_array = np.array(new_depth,  dtype=np.float32) 
                if len(depth_array.shape) == 2:  # 确认是否为二维数组
                    depth_array = np.stack((depth_array,) * 1, axis=-1)
                new_depths.append(depth_array)
            data["depth_map"] = new_depths

        if  data.get("semantic_map") != None:
            semantics = data["semantic_map"]
            new_semantics = []
            for semantic in semantics:
                resize, resize_dims, crop, flip, rotate = self.sample_augmentation(data)
                new_semantic1 =  cv2.resize(semantic, resize_dims, interpolation=cv2.INTER_NEAREST)  
                new_semantic = new_semantic1[ crop[1]:crop[3], crop[0]:crop[2] ]
                semantic_array = np.array(new_semantic,  dtype=np.float32) 
                if len(semantic_array.shape) == 2:  # 确认是否为二维数组
                    semantic_array = np.stack((semantic_array,) * 1, axis=-1)
                new_semantics.append(semantic_array)
            data["semantic_map"] = new_semantics
            
        if  data.get("occ") != None:
            1
...
class GridMask:
    ...
    def __call__(self, results):
        ...
        lidars = results["lidar"]
        if self.offset:
            offset = torch.from_numpy(2 * (np.random.rand(h, w) - 0.5)).float()
            offset = (1 - mask) * offset
            lidars = [x * mask + offset for x in lidars]
        else:
            lidars = [x * mask for x in lidars]
        results.update(lidar=lidars)
        
        semantic_maps = results["semantic_map"]
        if self.offset:
            offset = torch.from_numpy(2 * (np.random.rand(h, w) - 0.5)).float()
            offset = (1 - mask) * offset
            semantic_maps = [x * mask + offset for x in semantic_maps]
        else:
            semantic_maps = [x * mask for x in semantic_maps]
        results.update(semantic_map=semantic_maps)
        
        depth_maps = results["depth_map"]
        if self.offset:
            offset = torch.from_numpy(2 * (np.random.rand(h, w) - 0.5)).float()
            offset = (1 - mask) * offset
            depth_maps = [x * mask + offset for x in depth_maps]
        else:
            depth_maps = [x * mask for x in depth_maps]
        results.update(depth_map=depth_maps)
...
class ObjectPaste:
    ...
    def __call__(self, data):
        ...
        if self.sample_2d:
            ...
            sampled_dict = self.db_sampler.sample_all(
                gt_bboxes_3d.tensor.numpy(), gt_labels_3d, img=None,lidar=None
            )
        ...
        if sampled_dict is not None:
            ...
            if self.sample_2d:
                ...
                data["lidar"] = sampled_dict["lidar"]
                data["semantic_map"] = sampled_dict["semantic_map"]
                data["depth_map"] = sampled_dict["depth_map"]
...
class ReorderMultiViewImages:
    ...
    def __init__(self, order, safe=True, dataset="nuscenes"):
        self.dataset = dataset
        if  self.dataset == "nuplan":
            self.ORI_ORDER = ['CAM_F0', 'CAM_L0', 'CAM_R0', 'CAM_L1',   'CAM_R1', 'CAM_L2',  'CAM_R2', 'CAM_B0']
            
            self.REORDER_KEYS = ["image_paths", "lidar2camera", "lidar2image",   "camera_intrinsics",  "camera2lidar", "filename", "img","lidar","img_aug_matrix","depth_map", "semantic_map"]
        else:  
            self.ORI_ORDER = ["CAM_FRONT","CAM_FRONT_RIGHT", "CAM_FRONT_LEFT", "CAM_BACK", "CAM_BACK_LEFT", "CAM_BACK_RIGHT",]
            self.REORDER_KEYS = ["image_paths", "lidar2camera", "lidar2image", "camera2ego", "camera_intrinsics",  "camera2lidar", "filename", "img","lidar","img_aug_matrix","depth_map", "semantic_map"]
        self.order = order
...
class ImagePad:
    ...
    def _pad_img(self, results):
        ...
        if self.size is not None:
            ...
            padded_lidar = [
                mmcv.impad(lidar, shape=self.size, pad_val=self.pad_val)
                for lidar in results["lidar"]
            ]
            padded_semantic = [
                mmcv.impad(semantic, shape=self.size, pad_val=self.pad_val)
                for semantic in results["semantic_map"]
            ]
            padded_depth = [
                mmcv.impad(depth, shape=self.size, pad_val=self.pad_val)
            ...
            padded_lidar = [
                mmcv.impad_to_multiple(lidar, self.size_divisor, pad_val=self.pad_val)
                for lidar in results["lidar"]
            ]
            
            padded_semantic = [
                mmcv.impad_to_multiple(semantic, self.size_divisor, pad_val=self.pad_val)
                for semantic in results["semantic_map"]
            ]
            padded_depth = [
                mmcv.impad_to_multiple(depth, self.size_divisor, pad_val=self.pad_val)
                for depth in results["depth_map"]
            ]
            
        results["img"] = padded_img
        results["lidar"] = padded_lidar
        results["semantic_map"] = padded_semantic
...
class ImageNormalize:
    def __init__(self, mean, std):
        ...
        self.compose2 = torchvision.transforms.Compose(
            [
                torchvision.transforms.ToTensor(),
            ]
        )
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if data.get("lidar") !=None:
            data["lidar"] = [  ((torch.tensor(np.array(lidar)).permute(2, 0, 1)/120.0)*2.0 -1.0)  for lidar in data["lidar"]  ]
        if data.get("occ") !=None:
            data["occ"] = [  (torch.tensor(np.array(occ)))  for occ in data["occ"]  ]
        
        if data.get("semantic_map") !=None:
            data["semantic_map"] = [  (torch.tensor(np.array(semantic)).permute(2, 0, 1))  for semantic in data["semantic_map"]  ]
        if data.get("depth_map") !=None:
            # print(data["depth_map"][0].max(), data["depth_map"][0].min(),)
            data["depth_map"] = [  ((torch.tensor(np.array(depth+1e-4)).permute(2, 0, 1)/100.0))  for depth in data["depth_map"]  ]
