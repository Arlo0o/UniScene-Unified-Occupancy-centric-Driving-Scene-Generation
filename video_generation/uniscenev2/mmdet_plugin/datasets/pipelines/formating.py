...
class DefaultFormatBundle3D:
    ...
    def __call__(self, results):
        ...
        if "lidar" in results:
            results["lidar"] = DC(torch.stack(results["lidar"]), stack=True)
        if "depth_map" in results:
            results["depth_map"] = DC(torch.stack(results["depth_map"]), stack=True)
        if "semantic_map" in results:
            results["semantic_map"] = DC(torch.stack(results["semantic_map"]), stack=True)
        if "occ" in results:
            results["occ"] = DC(torch.stack(results["occ"]), stack=True)
