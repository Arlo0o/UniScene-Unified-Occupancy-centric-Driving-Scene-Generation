...
from uniscenev2_video.registry import DATASETS
from mmcv.parallel import DataContainer
from uniscenev2_video.mmdet_plugin.datasets import NuplanDataset
...
def _tokenize_captions(examples, template, tokenizer=None, is_train=True):
    ...
    for example in examples:
        ...
        tmpdata=example["metas"].data['description']
        description = f"{tmpdata['map_location']}, {tmpdata['llm_description']}"
        caption = template.format(description=description)
...
def collate_fn_single_clip(
    ...
    if examples[0].get("lidar") !=None:
        lidar_values = torch.stack([example["lidar"].data for example in examples])
        lidar_values = lidar_values.to( memory_format=torch.contiguous_format).float()
    
    if examples[0].get("depth_map") !=None:
        depth_values = torch.stack([example["depth_map"].data for example in examples])
        depth_values = depth_values.to( memory_format=torch.contiguous_format).float()
    if examples[0].get("semantic_map") !=None:
        semantic_values = torch.stack([example["semantic_map"].data for example in examples])
        semantic_values = semantic_values.to( memory_format=torch.contiguous_format).float()
    if examples[0].get("occ") !=None:
        occ_values = torch.stack([example["occ"].data for example in examples])
        occ_values = occ_values.to( memory_format=torch.contiguous_format).float()
    ...
    if examples[0].get("depth_map") !=None:
        ret_dict["depth_map"] = depth_values
    if examples[0].get("semantic_map") !=None:
        ret_dict["semantic_map"] = semantic_values
    if examples[0].get("occ") !=None:
        ret_dict["occ"] = occ_values
    if examples[0].get("lidar_values") !=None:
        ret_dict["lidar_values"] = lidar_values
...
class NuplanTDataset(NuplanDataset):
    def __init__(
        self,
        ann_file,
        pipeline=None,
    ...
    def replace_path(self,frame,name):
        for key in frame.keys():
            if name in key:
                if isinstance(frame[key],str):
                    frame[key] = frame[key].replace('../','')
                elif isinstance(frame[key],list):
                    new_path_list = []
                    for current_path in frame[key]:
                        new_path_list.append(current_path.replace('../',''))
                    frame[key]=new_path_list
        return frame
    ...
    def load_frames(self, frames):
        ...
        for frame in frames:
            frame = self.replace_path(frame,'path')
