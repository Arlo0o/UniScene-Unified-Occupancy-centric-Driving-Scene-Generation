...
from uniscenev2_video.datasets.dataset import NuScenesMultiResDataset
from uniscenev2_video.datasets.sampler import StatefulDistributedSampler, NuScenesVariableBatchSampler


from uniscenev2_video.datasets.dataset_nuplan import NuplanMultiResDataset
from uniscenev2_video.datasets.nuplan_base_datasets import NuplanTDataset ###
...
def prepare_dataloader(
    ...
    **kwargs,
    ...
    _kwargs = kwargs.copy()
    print( "************************" , dataset)
    if isinstance(dataset, NuplanMultiResDataset):
    ...
    elif isinstance(dataset, NuScenesMultiResDataset):
        process_group = process_group or _get_default_group()
        batch_sampler = NuScenesVariableBatchSampler(
            dataset,
            bucket_config,
    ...
    elif isinstance(dataset, (NuplanTDataset, Subset)):
        process_group = process_group or _get_default_group()
        sampler = StatefulDistributedSampler(
            dataset,
            num_replicas=process_group.size(),
