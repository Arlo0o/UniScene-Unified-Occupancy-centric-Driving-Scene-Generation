...
class LoadMultiViewImageFromFiles:
    ...
    def __init__(self, to_float32=False, color_type="unchanged", use_lidar=False, img_path=None, seg_path=None, depth_path=None, occ_path=None ):
    ...
    def __call__(self, results):
        ...
        """
        filename = [os.path.join(self.img_path, path) for path in results["image_paths"]]
        # print("results[image_paths]:  -------", results["image_paths"])
        ...
        if self.use_lidar:
            for name in filename:
                lidar_name = name.split(".")[0] + '_lidar_4.npy'
                np_array = np.load(lidar_name)
                lidar_images.append(np_array)
            results["lidar"] = lidar_images
        # else:
        #     results["lidar"] = [Image.fromarray( np.zeros((1920,1080,3)) , mode='RGB')]*len(filename)
            
            
        if  self.occ_path != None:
            # occ_path = os.path.join(self.occ_path, results['token'], results['token'] + ".npz")
            # if os.path.exists(occ_path):
            #     occ = np.load(occ_path)['occ']
            #     occs.append(occ)
            occ_path = os.path.join(self.occ_path,  results['token'] + ".npy")
            if os.path.exists(occ_path):
                occ = np.load(occ_path)
                occs.append(occ)
                
            else:
                np_array = np.zeros( (200,200,16) )
                # print(occ_path, "None occ----->")
        if self.depth_path != None and self.seg_path != None:
            for image_path in filename:
                parts = image_path.split("/")  # 按照"/"分割路径
                cam_part = parts[-2]  # 提取倒数第二个部分（即CAM_B0）
                # cam_b0_part = image_path[image_path.rfind(cam_part):]  # 从CAM_B0开始到路径末尾
                
                # depth_path = f"{self.depth_path}/{cam_b0_part.replace('.jpg', '/depth_data.npz')}"
                depth_path = os.path.join( self.depth_path, cam_part, results['token'] , 'depth_data.npz')
                if os.path.exists(depth_path):
                    # print( "get----->",   depth_path)
                    try:
                        np_array = np.load(depth_path)['arr_0']
                    except:
                        np_array = np.zeros((1920,1080))
                        print(image_path, "Wrong----->", depth_path)
                else:
                    np_array = np.zeros((1920,1080))
                    print(image_path, "None----->", depth_path)
                depth_images.append(np_array)
                
                
                seg_path =  os.path.join( self.seg_path, cam_part, results['token'] , 'semantic.npz') #f"{self.seg_path}/{cam_b0_part.replace('.jpg', '/semantic.npz')}"
                if os.path.exists(seg_path):
                    try:
                        np_array = np.load(seg_path)['arr_0']
                    except:
                        np_array = np.zeros((1920,1080))
                else:
                    np_array = np.zeros((1920,1080))
                seg_images.append(np_array)
                
                
            results["depth_map"] = depth_images
            results["semantic_map"] =  seg_images
            if  self.occ_path != None:
                results["occ"] = occs
