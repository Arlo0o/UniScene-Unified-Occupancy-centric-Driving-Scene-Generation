...
from uniscenev2_video.models.layers.blocks import Attention, get_layernorm, approx_gelu, t2i_modulate
from uniscenev2_video.models.vae.CogVAE3D.vae_temporal import CausalConv3d, pad_at_dim
from uniscenev2_video.models.utils import zero_module
...
class RayMapControlTempEmbedding(nn.Module):
...
class RayMapControlEmbedding(nn.Module):
    def __init__(
        self,
        conditioning_embedding_channels: int = 320,
        conditioning_size: int = 6,  # only use 25
        block_out_channels: Tuple[int, ...] = (32, 64, 128, 256),
        drop_cond_ratio: float = 0.15,
    ):
        super().__init__()
        ...
        for i in range(len(block_out_channels) - 2):
            ...
            # )
            self.blocks.append(
                nn.Conv2d(channel_in, channel_out, kernel_size=3, padding=(1, 1), stride=2))
        channel_in = block_out_channels[-2]
        channel_out = block_out_channels[-1]
        # self.blocks.append(
        #     nn.Conv2d(channel_in, channel_in, kernel_size=3, padding=(1, 1)))
        self.blocks.append(
            nn.Conv2d( channel_in, channel_out, kernel_size=3, padding=(1, 1),  stride=(2, 2)))
    ...
    def substitute_with_uncond_map(self, controlnet_cond, mask=None):
        ...
        controlnet_cond[mask > 0] = torch.zeros_like(controlnet_cond[mask > 0])
        return controlnet_cond
    ...
    def forward(self, controlnet_cond, drop=False):
        BNC,C,T,H,W = controlnet_cond.shape
        if drop:
            controlnet_cond = self._random_use_uncond_map(controlnet_cond)
        controlnet_cond = rearrange(controlnet_cond, "BNC C T ... -> (BNC T) C ... ")
        # embedder module
        ...
        return rearrange(embedding, "(BNC T) ... -> BNC T ... ", BNC=BNC,T=T)
...
if __name__ == "__main__":
    ...
    m = RayMapControlEmbedding(
        conditioning_embedding_channels=576,
        conditioning_size=1,
        block_out_channels=[16, 32, 96, 256],
    ).to(device).to(torch.bfloat16)

    x = torch.rand(48,1,17,224,400).to(device,torch.bfloat16)
    ...
    rm = RayMapControlTempEmbedding(
        hidden_size=1152,
        time_downsample_factor=4.5
    ).to(device).to(torch.bfloat16)
    x2 = rearrange(res, "BNC T C ... -> BNC C T ... ") 
    # x2 = rearrange(res, "B NC T C ... -> (B NC) C T ... ") 
    #$torch.rand(6, 576, 17, 28, 50).to(device,torch.bfloat16)

    embed()
    exit()
    # x torch.Size([6, 16, 9, 44, 88])
    with torch.no_grad():
        res = rm(x2)

    # torch.Size([6, 1152, 5, 28, 50])
