...
from uniscenev2_video.acceleration.parallel_states import get_data_parallel_group
from uniscenev2_video.acceleration.communications import gather_tensors, serialize_state, deserialize_state
...
def load(
    ...
    if ema is not None:
        ...
        try:
            ...
            print(e)
            ema_pt = torch.load(os.path.join(load_dir, "ema.pt"), map_location=torch.device("cpu"))
            network_dict = ema.state_dict()
            for name, param in ema_pt.items():
                if name in network_dict.keys():
                    if ema_pt[name].shape == network_dict[name].shape:
    ...
    #             if name in network_dict.keys():
    #                 if param.shape == network_dict[name].shape:
    #                     network_dict[name].copy_(param)
    #                 else:
    #                     logging.warning(f"Shape mismatch for {name}, skipping.")
    #             else:
    #                 logging.warning(f"Parameter {name} not found in EMA model, skipping.")
    #         ema.load_state_dict(network_dict, strict=False)
    #     except Exception as e:
    #         logging.exception(f"Error loading EMA state dict: {e}")
    #         logging.warning("Failed to load EMA weights due to an error. Continuing without EMA.")
    #     # 确保即使在加载失败的情况下也更新EMA，以防需要从头开始或用模型当前状态更新
    #     update_ema(ema, model.module, decay=0, sharded=False)
