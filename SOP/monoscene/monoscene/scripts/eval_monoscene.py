...
import shutil
sys.path.append('./')
shutil.copy('gen-efficientnet-pytorch/tf_efficientnet_b7_ns-1dbc32de.pth', '/root/.cache/torch/hub/checkpoints/tf_efficientnet_b7_ns-1dbc32de.pth')
ABS_WORK_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../"))
...
@hydra.main(config_name="monoscene_nuplan_eval_1gpu.yaml", config_path="../config")
def main(config: DictConfig):
    ...
    if config.dataset == "kitti":
        ...
        data_module = NuplanDataModule(
            root=os.path.join(ABS_WORK_DIR, 'data/nuplan_all/sensor_blobs/mini'),
            pkls_path=os.path.join(ABS_WORK_DIR, 'data/nuplan_pkls'),
            occ_path=os.path.join(ABS_WORK_DIR, 'data/occ_quan/nuplan_quantized_400_400_32'),
            frustum_size=config.frustum_size,
    ...
    if config.dataset == "NYU":
        ...
        model_path = os.path.join(ABS_WORK_DIR, 'mIoU=0.09392.ckpt')
    else:
