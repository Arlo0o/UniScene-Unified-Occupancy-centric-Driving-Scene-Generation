...
def eval_one_epoch(cfg, args, model, dataloader, epoch_id, logger, dist_test=False, result_dir=None, result_tag=""):
    ...
    pred_path = os.path.join(args.work_dir, f'pc_seq_{args.selected_seq}_{args.lidar_chosen}', 'pred')
    gt_path = os.path.join(args.work_dir, f'pc_seq_{args.selected_seq}_{args.lidar_chosen}', 'gt')
