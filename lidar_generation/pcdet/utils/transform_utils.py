...
def cartesian_to_spherical(points, to_deg=False):
    thetas = torch.asin(points[:, -1] / torch.sqrt(torch.linalg.norm(points, dim=-1)))
    phis = torch.atan2(points[:, 1], points[:, 0])
    if to_deg:
        thetas = torch.rad2deg(thetas)
        phis = torch.rad2deg(phis)
    return torch.stack([phis, thetas], dim=-1)
