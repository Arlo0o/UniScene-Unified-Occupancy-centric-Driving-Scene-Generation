...
class NuPlanOccDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        self.cylindrical_voxelize = dataset_cfg.get('cylindrical_voxelize', False)
    ...
    def __getitem__(self, idx):
        ...
        xyz_quan = ((data_dict['occ'][:, :3] - pc_range) / voxel_size)
        if not self.cylindrical_voxelize:
            ...
            polar_res = np.stack([rho, phi, xyz[:, 2]], axis=-1)
            polar_range = [0, -np.pi, self.point_cloud_range[2], math.sqrt(((self.point_cloud_range[3]-self.point_cloud_range[0])/2)**2+ \
                                ((self.point_cloud_range[4]-self.point_cloud_range[1])/2)**2), np.pi, self.point_cloud_range[-1]]
            polar_size = [(polar_range[3]-polar_range[0])/self.grid_size[0], (polar_range[4]-polar_range[1])/self.grid_size[1], (polar_range[5]-polar_range[2])/self.grid_size[2]]
            polar_range = np.array(polar_range, dtype='float32').reshape((-1, 6))
            polar_size = np.array(polar_size, dtype='float32').reshape((-1, 3))
            data_dict['occ'][:, :3] = (polar_res - polar_range[:,:3]) / polar_size
        ...
        xyz_quan = xyz_quan[occ_range_mask]
        occ_labels = data_dict['occ'][:, -1]
        xyz_ = (xyz_quan + 0.5) * voxel_size + pc_range
        tpr = cartesian_to_spherical(xyz_)
        cls_encoded = np.eye(len(self.class_names))[occ_labels]
        occ_feature = np.concatenate([xyz_, tpr, cls_encoded], axis=-1)
        ...
        xyz = xyz_quan.astype(np.int32)
        if self.cylindrical_voxelize:
            xyz = xyz[(xyz_quan[:,0]>=0)&(xyz_quan[:,1]>=0)&(xyz_quan[:,2]>=0)&\
                    (xyz_quan[:,0]<self.grid_size[0])&(xyz_quan[:,1]<self.grid_size[1])&(xyz_quan[:,2]<self.grid_size[2])]
