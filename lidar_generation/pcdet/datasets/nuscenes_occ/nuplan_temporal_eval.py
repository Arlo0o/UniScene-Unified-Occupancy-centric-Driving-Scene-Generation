...
class NuPlanOccDatasetTemporalEval(NuPlanOccDataset):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        self.times = 1 # 200
        self.per_scene_len = [len(item) // self.return_len for item in self.full_list]
        self.accum_len = np.cumsum(np.array(self.per_scene_len))
        self.accum_len = np.concatenate([np.array([0], dtype=self.accum_len.dtype), self.accum_len])

    def __len__(self):
        # return len(self.full_list) * self.times
        return sum(self.per_scene_len)
    ...
    def load_infos(self, info_file):
        ...
        if self.selected_seq is not None:
            # 取一个序列
            infos['scene_tokens'] = [infos['scene_tokens'][0]]
    ...
    def __getitem__(self, idx):
        # 判断是哪个序列
        scene_idx = np.searchsorted(self.accum_len, idx, side='right') - 1
        idx -= self.accum_len[scene_idx]
        sample_tokens = self.full_list[scene_idx]
        scene_len = len(sample_tokens)
        # start_idx = np.random.randint(0, scene_len - self.return_len - self.offset + 1)
        start_idx = idx * self.return_len
        data_dict_all = []
        for idx in range(start_idx, min(start_idx + self.return_len + self.offset, len(sample_tokens))):
            input_dict = {}
            ...
            if (idx > start_idx) and self.training:
