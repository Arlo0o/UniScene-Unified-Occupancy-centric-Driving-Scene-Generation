...
class Occ2LiDARDatasetNKSRGenVideo(Occ2LiDARDataset):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        # some examples
        #self.full_list = ['0c223ae827e44e55a13833ca984ca4c3','0f546bbb1e9048a1b84f70fb4aedf9a0', '1a6770bd0f364497b5b2642af95b6025', '0a535567072a48a38a7213778550dcaf', '0d0f1e7f3aaa4fbb931f1f5aecc3b9d8']
        #self.full_list = ['0a535567072a48a38a7213778550dcaf']
        self.video_root = 'vis_1119/vis_concat80m_withtra'
        self.full_list = glob('vis_1119/vis_concat80m_withtra/*/Big_occ.npz')
        #self.full_list = ['vis_1119/vis_concat80m/0a535567072a48a38a7213778550dcaf/Big_occ.npz']
        ...
        # self.full_list, self.video_ids, self.relative_poses = self.get_video_tokens(self.full_list)
        # self.length = len(self.full_list)
        self.gap = dataset_cfg.gap
        # self.trajectory_dict = {
        #     # pad 100
        #     '0a535567072a48a38a7213778550dcaf': [[120+i+100,140] for i in range(-120, 280, self.gap)]
        # }
        self.trajectory_dict = []
        total_frames = 0
        self.idx_to_tra = []
        scene_cnt = 0
        for i, p in enumerate(self.full_list):
            tra_path = os.path.join(os.path.dirname(p), 'tra.npy')
            if not os.path.exists(tra_path):# or os.path.exists(os.path.join('exps/vis_video_suppl', tra_path.split('/')[-2]+f'_{self.gap}'+'.bin')):
                continue
            tra = np.load(tra_path)
            tra = tra[:, [2, 1]].astype(int).tolist()
            tra = tra[::2]
            valid_tra = []
            for t in tra:
                if t[0]-100>=0 and t[0]+100<=600 and t[1]-100>=0 and t[1]+100<=400:
                    valid_tra.append(t)
            if len(valid_tra) < 20:
                continue
            self.trajectory_dict.append([p.split('/')[-2], valid_tra])
            #self.trajectory_dict.append([p.split('/')[-2], [[120+i+100,140] for i in range(-120, 280, self.gap)]])
            total_frames += len(self.trajectory_dict[-1][-1])
            for j in range(len(self.trajectory_dict[-1][-1])):
                self.idx_to_tra.append([scene_cnt, j])
            scene_cnt += 1
        #self.length = self.trajectory_dict[self.full_list[0]].__len__()
    ...
    def load_infos(self, info_file):
        ...
        for i, item in enumerate(infos):
            ...
            self.seq_id_to_info_map[i] = [{'token': item['token'], 'cams': item['cams']} for item in seq_infos]#seq_infos
    ...
    def get_video_tokens(self, start_tokens):
        full_list_video = []
        video_ids = []
        relative_poses = []
        for i, start_token in enumerate(start_tokens):
            seq_id = self.token_to_seq_id_map[start_token]
            seq_infos = self.seq_id_to_info_map[seq_id]
            poses = self.seq_id_to_poses[seq_id]
            seq_tokens = [info['token'] for info in seq_infos]
            video_tokens = seq_tokens[seq_tokens.index(start_token)::5]
            full_list_video.extend(video_tokens)
            video_ids.extend([i for _ in range(len(video_tokens))])
            video_poses = poses[seq_tokens.index(start_token)::5]
            video_infos = seq_infos[seq_tokens.index(start_token)::5]

            # import shutil
            # for i in range(len(video_infos)):
            #     cam_path = video_infos[i]['cams']['CAM_FRONT']['data_path']
            #     cam_path = os.path.join('data/nuScenes/v1.0-trainval', *cam_path.split('/')[-3:])
            #     os.makedirs('vis_video', exist_ok=True)
            #     shutil.copyfile(cam_path, f'vis_video/{i}.png')

            _relative_pose = []
            for pose in video_poses:
                # lidar1 -> world -> lidar0
                _relative_pose.append(torch.linalg.inv(video_poses[0]) @ pose)
            relative_poses.extend(_relative_pose)
            
        return full_list_video, video_ids, relative_poses

    def __getitem__(self, idx):
        input_dict = {}
        #occ_filename = self.full_list[0]
        ...
        #input_dict['frame_id'] = self.full_list[0]+f'_{idx}'
        #occ_filepath = os.path.join('data/nuScenes/concat', self.full_list[0], 'Big_occ.npz')

        i_, j_ = self.idx_to_tra[idx]
        frame_id = self.trajectory_dict[i_][0]
        input_dict['frame_id'] = frame_id
        occ_filepath = os.path.join(self.video_root, frame_id, 'Big_occ.npz')
        occ = np.load(occ_filepath)['occ']
        occ[occ==17] = 0
        occ = np.pad(occ, ((100, 100), (0, 0), (0, 0)), mode='constant')
        ego_point = self.trajectory_dict[i_][1][j_]
        occ = occ[int(ego_point[0])-100:int(ego_point[0])+100, int(ego_point[1])-100:int(ego_point[1])+100, :]
        ...
        if j_ == (len(self.trajectory_dict[i_][1])-1):
            data_dict['tra'] = self.trajectory_dict[i_][1]
            data_dict['end_flag'] = True
        return data_dict
