...
def obtain_points_label(occ):
    ...
    return points, coords, points_label

class OpenSceneDataset(Occ2LiDARDataset):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        self.cls_map = {1: 4, 2: 12, 3: 12, 4: 12, 5: 12, 6: 2, 7: 1, 8: 7, 9: 8, 10: 1, 11: 14}
        self.data_root = self.occ_root#'data/openscene/openscene-v1.0/occupancy/mini/log-0001-scene-0001/occ_gt'
        self.full_list = os.listdir(self.data_root)
        self.full_list = list(filter(lambda x: 'flow' not in x, self.full_list))
        self.length = len(self.full_list)
    ...
    def __getitem__(self, idx):
        ...
        occ_gt = np.load(os.path.join(self.data_root, occ_filename))
        _, occ_loc, labels = obtain_points_label(occ_gt)
        
        labels = labels + 1
        new_labels = np.zeros_like(labels)
        for k, v in self.cls_map.items():
        ...
        occ = np.concatenate([occ_loc[:, [2, 1, 0]], labels[:, None]+1], axis=-1)
        # !!! 没有用到 随便弄一个
        ...
        input_dict['occ'][:, :3] = (input_dict['occ'][:, :3] + 0.5) * voxel_size + np.array(point_cloud_range[:3]).reshape((-1, 3))#pc_range
        # openscene occ坐标在地面 移到lidar上
        input_dict['occ'][:, 2] -= 1.7
