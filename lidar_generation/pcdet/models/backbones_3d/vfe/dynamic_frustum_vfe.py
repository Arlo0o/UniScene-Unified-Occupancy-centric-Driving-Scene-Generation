...
class DynamicFrustumVFE(VFETemplate):
    def __init__(self, model_cfg, num_point_features, voxel_size, grid_size, point_cloud_range, **kwargs):
        super().__init__(model_cfg=model_cfg)
        num_point_features = self.model_cfg.get('NUM_POINT_FEATURES', num_point_features)
        point_cloud_range = np.array([-torch.pi, 0, 0, torch.pi, torch.pi, 100])
        voxel_size = np.array([0.01*torch.pi, 0.01*torch.pi, 1])
