...
class Occ2LiDARRenderBatch(nn.Module):
    def __init__(
        self,
        model_cfg,
        # in_channels,
    ...
    def forward(self, batch_dict):#pts_feats, rays):
        ...
        batch_ret = []
        rays = self.sample_rays(pts, did_returns, None, None, occ_grids=occ_grids, sensor_locs=sensor_locs)
        lidar_rays, _ = rays

        # batchify lidar_rays
        n_downsample = min([item['ray_o'].shape[0] for item in lidar_rays])
        batch_lidar_rays = defaultdict(list)
        for bs in range(batch_size):
            sampled_idx = torch.randperm(lidar_rays[bs]['ray_o'].shape[0])[:n_downsample]
            for k, v in lidar_rays[bs].items():
                if isinstance(v, torch.Tensor):
                    batch_lidar_rays[k].append(lidar_rays[bs][k][sampled_idx])
                else:
                    batch_lidar_rays[k] = v
        for k, v in batch_lidar_rays.items():
            if isinstance(v, list):
                batch_lidar_rays[k] = torch.stack(v)
        lidar_rays = batch_lidar_rays
        ...
        if self.training:
            scaled_points = lidar_rays["scaled_points"]
            lidar_ray_bundle = RayBundle(
                origins=lidar_rays["ray_o"], directions=lidar_rays["ray_d"], depths=lidar_rays.get("depth", None)
            )
            cam_ray_bundle = None
            preds_dict = self.render_model(
                lidar_ray_bundle, pts_feats.contiguous(), points=scaled_points
            )
            ...
            preds_dict = self.render_model(
                lidar_ray_bundle, pts_feats.contiguous()
            )

        # for i in range(preds_dict['depth'].shape[0]):
        #     batch_ret.append(dict(depth=preds_dict['depth'][i], lidar_relative=preds_dict['lidar_relative'][i]))
        self.forward_ret_dict['preds_dict'] = [preds_dict]
    ...
    def sample_rays(self, pts, did_returns, imgs, img_metas, occ_grids=None, sensor_locs=None):
        #if self.training:
        lidar_ret = self.sample_lidar_rays(pts, did_returns, img_metas, occ_grids=occ_grids, sensor_locs=sensor_locs)
        #else:
    ...
    def sample_lidar_rays(self, pts, did_returns, img_metas, test=False, occ_grids=None, sensor_locs=None):
        """Get lidar ray
        Returns:
