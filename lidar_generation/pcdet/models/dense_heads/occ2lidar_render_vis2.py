...
class Occ2LiDARRenderVis2(nn.Module):
    def __init__(
        self,
        model_cfg,
        # in_channels,
        ...
        **kwargs
        ...
        super().__init__()
        self.pred_save_path = model_cfg.pred_save_path
    ...
    def generate_predicted_pc(self, batch_dict):
        ...
        for bs in range(batch_size):
            ...
            if raydrop_pred is not None and self.drop_ray_when_pred:
                assert not self.use_gt_drop and len(batch_dict['frame_id']) == 1
                if self.use_gt_drop:
                    pred_pts = pred_pts[lidar_rays[bs]['did_return'].squeeze()]
                else:
                    pred_pts1 = pred_pts[(raydrop_pred.sigmoid()<1.0).squeeze()]
                    pred_pts2 = pred_pts[(raydrop_pred.sigmoid()<0.9).squeeze()]
                    pred_pts3 = pred_pts[(raydrop_pred.sigmoid()<0.8).squeeze()]
                    os.makedirs(os.path.join(self.pred_save_path, 'drop1.0'), exist_ok=True)
                    os.makedirs(os.path.join(self.pred_save_path, 'drop0.9'), exist_ok=True)
                    os.makedirs(os.path.join(self.pred_save_path, 'drop0.8'), exist_ok=True)
                    pred_pts1.cpu().numpy().astype('float32').tofile(os.path.join(self.pred_save_path, 'drop1.0', batch_dict['frame_id'][0]+'.bin'))
                    pred_pts2.cpu().numpy().astype('float32').tofile(os.path.join(self.pred_save_path, 'drop0.9', batch_dict['frame_id'][0]+'.bin'))
                    pred_pts3.cpu().numpy().astype('float32').tofile(os.path.join(self.pred_save_path, 'drop0.8', batch_dict['frame_id'][0]+'.bin'))
    ...
    def forward(self, batch_dict):#pts_feats, rays):
        ...
        batch_ret = []
        rays = self.sample_rays(pts, did_returns, None, None, occ_grids=occ_grids)
        lidar_rays, _ = rays
