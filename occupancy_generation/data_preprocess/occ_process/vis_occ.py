...
occ_root = 'data/nuplan/GT_occ_fast/dense_voxels_with_semantic'
...
pkl_file = '/lpai/dataset/nuplan-pkl/25-03-10-1/mini/nuplan_mini_10hz_train.pkl'
...
def render_one_scene(scene_id, sample_tokens):
    ...
    writer = imageio.get_writer(f'vis/occ_preview/{scene_id}.mp4', fps=10//downsample_rate)
    figure = mlab.figure(size=(W, H), bgcolor=(1, 1, 1))
    for i, sample_token in enumerate(sample_tokens[::downsample_rate]):
        ...
        voxels_[:, -1] += 1
        mlab.clf()
        draw(voxels_, voxel_size=2, figure=figure, mlab=mlab)
