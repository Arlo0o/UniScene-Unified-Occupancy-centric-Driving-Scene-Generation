...
class QuantizationProcessor:
    def __init__(self, config_path, pkl_path, data_dir, output_dir, target_size, method, workers):
        # 初始化配置
    ...
    def _discover_files(self, pkl_path, data_dir):
        """发现所有需要处理的npz文件"""
        # pkl_path = "/mnt/dataset/nuplan-pkl/25-03-08-1/nuplan_pkls/nuplan_10hz_pkl/trainval/nuplan_trainval_10hz_val.pkl"
    ...
    def _process_single(self, file_path):
        ...
        try:
            # 加载并转换标签
            orig_labels = load_occ_gt(file_path, grid_size=(400,400,32), voxel_size=(0.25,0.25,0.25))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config_path', type=str, required=True, help='YAML配置文件路径')
    parser.add_argument('--pkl_path', type=str, required=True, help='pkl文件目录')
    parser.add_argument('--data_base_path', type=str, required=True, help='输入数据目录')
    parser.add_argument('--save_base_path', type=str, required=True, help='输出目录')
    parser.add_argument('--quantize_size', type=int, nargs=3, required=True, help='目标尺寸 (X Y Z)')
    parser.add_argument('--method', choices=['max', 'nearest'], default='max', help='降采样方法')
    parser.add_argument('--workers', type=int, default=cpu_count(), help='并行工作进程数')
