...
class QuantizationProcessor:
    def __init__(self, config_path, data_dir, output_dir, target_size, method, workers):
        # 初始化配置
        ...
        # 获取文件列表并过滤已处理
        self.file_list = self._discover_files(data_dir)
    ...
    def _discover_files(self, data_dir):
        """发现所有需要处理的npz文件"""
        pkl_path = "/data/longhun/3D/nuplan/Nuplan-Occupancy/pickle/mini/nuplan_mini_10hz_val.pkl"
    ...
    def _process_single(self, file_path):
        ...
        try:
            # 加载并转换标签
            orig_labels = load_occ_gt(file_path)
