...
def scaled_dot_product_attention_chunk(q, k, v, mask=None, attn_dropout=0., training=True, chunk_size=128):
    attn_list = []
    
    # 遍历长序列，按块计算
    for start_idx in range(0, q.shape[2], chunk_size):
        end_idx = min(start_idx + chunk_size, q.shape[2])
        
        q_chunk = q[:, :, start_idx:end_idx, :]
        k_chunk = k[:, :, start_idx:end_idx, :]
        v_chunk = v[:, :, start_idx:end_idx, :]
        
        attn = torch.matmul(q_chunk, k_chunk.transpose(-1, -2))  # (b, n_head, seq_len_chunk, seq_len_chunk)
        attn = attn / np.sqrt(q.shape[-1])
        ...
        a_chunk = torch.matmul(attn, v_chunk)
        attn_list.append(a_chunk)

    # 合并分块结果
    a = torch.cat(attn_list, dim=-2)
