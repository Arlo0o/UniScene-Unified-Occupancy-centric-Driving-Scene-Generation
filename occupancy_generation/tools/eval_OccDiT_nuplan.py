...
def main(args):
    ...
    diffusion = create_diffusion('')  # default: 1000 steps, linear noise schedule
    ...
    bz = int(args.dit_batch_size // dist.get_world_size())
    # bz=1
    if vis:
        bz = 1
    ...
    # if not vis:
    log_file = os.path.join(log_path, f'Dit_eval_ddim_continuous_mVAE_{args.inversion}_{args.cfg_scale}_{lambda_np}.log')
    logger = MMLogger('genocc', log_file=log_file, log_level='INFO')
    MMLogger._instance_dict['genocc'] = logger
    logger.info(f'Cfg scale:{args.cfg_scale}')
    logger.info(f"gts_path: {gts_path} =={imageset}=={bev_path}")
    ...
    with torch.no_grad():
        ...
        for i_iter_val, (occ_ori,y,occ_meta, scene_metas) in enumerate(tqdm(loader)):
            if args.debug and i_iter_val > 50:
                break
            ...
            if vis:
                ...
                for i in range(Tframe):
                    ...
                    vis_matrix(z_ori_vis[0,i],zvis_root+f"{i_iter_val}_{i}_0_{args.inversion}.png")

                print(f"vis_root is {vis_root}")
                plt.savefig(vis_root + f"{i_iter_val}_wonull_cfg-{args.cfg_scale}_{args.inversion}.png")
                ...
                imageio.mimsave(vis_root + f"{args.inversion}_inversion_{i_iter_val}_{lambda_np}_{args.cfg_scale}.mp4",images,'mp4',fps = 4)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--result-dir", type=str, default="outputs/nuplan_occ_dit_eval")
