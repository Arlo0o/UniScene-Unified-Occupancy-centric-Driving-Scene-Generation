...
def main(args):
    ...
    with torch.no_grad():
        for i_iter_val, (input_occs, target_occs, metas) in enumerate(tqdm(val_dataset_loader)):
            ...
            input_occs = input_occs.cuda() # 1, 5, 200, 200, 16
            input_occs = input_occs[:,0:1]
            input_occs = input_occs.repeat(1,5,1,1,1)
            input_occs = fill_large_zero_height_regions_gpu(input_occs, target_layer=8, fill_class=1)
            target_occs = target_occs.cuda()
            data_time_e = time.time()
            
            result_dict = my_model(x=input_occs, metas=metas)
            # latent_z = my_model.encode(x=input_occs, metas=metas)
            # result_dict = my_model.generate(latent_z, input_shape=[1,5,200,200,16])
            ...
            if i_iter_val % print_freq == 0 and local_rank == 0:
                np.save(os.path.join(save_occ_dir, f'{epoch}_{i_iter_val}_input_occs.npy'), input_occs.cpu().numpy().astype(np.int8))
                np.save(os.path.join(save_occ_dir, f'{epoch}_{i_iter_val}_pred_occs.npy'), result_dict['sem_pred'].cpu().numpy().astype(np.int8))
