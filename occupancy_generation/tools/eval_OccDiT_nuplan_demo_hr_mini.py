...
def main(args):
    ...
    # 检查权重加载情况
    model_keys = set(Dit_model.state_dict().keys())
    ...
    if not missing_keys and not unexpected_keys:
        print("✓ All model parameters match checkpoint perfectly!")
    
    # 加载权重
    missing_keys_load, unexpected_keys_load = Dit_model.load_state_dict(state_dict, strict=False)
    
    if missing_keys_load:
        print(f"WARNING: {len(missing_keys_load)} keys were not loaded from checkpoint")
    if unexpected_keys_load:
        print(f"WARNING: {len(unexpected_keys_load)} unexpected keys in checkpoint were ignored")
    ...
    save_occ_root = f"{result_dir}/vis_continuous_{args.inversion}_{args.cfg_scale}/save_occ/"
    # os.makedirs(zvis_root, exist_ok=True)
    os.makedirs(save_occ_root, exist_ok=True)
    # 如果使用inversion，需要足够的扩散步数
    if args.inversion:
        diffusion = create_diffusion('ddim50')  # 50步DDIM，支持inversion
    else:
        diffusion = create_diffusion('ddim10')  # 10步DDIM，快速推理
    ...
    imageset = args.imageset or "data/nuplan_mini_val_clip_infos_dit.pkl"
    ...
    bev_path = args.bev_root or "data/nuplan_bev_400/mini"
    # bev_path = '/lpai/dataset/nuplan-bev/0-1-0/nuplan_bev/mini/train'
    gts_path = args.occ_root or "data/occ_quan/nuplan_quantized_400_400_32"

    # dataset = Nuplan_Occ_bev_Dataset_demo(imageset,gts_path,bev_path,bev_ch_use,meta_num=4,Tframe=Tframe)
    dataset = Nuplan_Occ_bev_HR_mini(imageset,gts_path,bev_path,bev_ch_use,meta_num=4,Tframe=Tframe)
    ...
    # 检查VAE权重加载情况
    vae_model_keys = set(my_VQVAE.state_dict().keys())
    vae_ckpt_keys = set(vae_ckpt['state_dict'].keys())
    
    vae_missing_keys = vae_model_keys - vae_ckpt_keys
    vae_unexpected_keys = vae_ckpt_keys - vae_model_keys
    
    print(f"\nLoading VAE checkpoint from: {resume_from}")
    print(f"VAE model has {len(vae_model_keys)} parameters")
    print(f"VAE checkpoint has {len(vae_ckpt_keys)} parameters")
    
    if vae_missing_keys:
        print(f"Missing keys in VAE checkpoint ({len(vae_missing_keys)}):")
        for key in sorted(vae_missing_keys):
            print(f"  - {key}")
    
    if vae_unexpected_keys:
        print(f"Unexpected keys in VAE checkpoint ({len(vae_unexpected_keys)}):")
        for key in sorted(vae_unexpected_keys):
            print(f"  - {key}")
    
    if not vae_missing_keys and not vae_unexpected_keys:
        print("✓ All VAE parameters match checkpoint perfectly!")
    
    # 加载VAE权重
    vae_missing_keys_load, vae_unexpected_keys_load = my_VQVAE.load_state_dict(vae_ckpt['state_dict'], strict=False)
    
    if vae_missing_keys_load:
        print(f"WARNING: {len(vae_missing_keys_load)} VAE keys were not loaded from checkpoint")
    if vae_unexpected_keys_load:
        print(f"WARNING: {len(vae_unexpected_keys_load)} unexpected VAE keys in checkpoint were ignored")

    my_VQVAE.eval()


    # from model_vae.VAE.AE_eval_400x400 import Autoencoder_2D_400x400
    # ae_eval = Autoencoder_2D_400x400(num_classes=18,expansion=4)
    ...
    # if not vis:
    log_file = os.path.join(log_path, f'Dit_eval_ddim_continuous_mVAE_{iter_num}_{args.cfg_scale}_{lambda_np}_{args.inversion}_hr_mini.log')
    ...
    Cal_fid_mmd = multi_step_fid_mmd(feature_shape=16384)
    ...
    with torch.no_grad():
        ...
        for i_iter_val, (occ_ori,y,occ_meta, pose_meta, scene_metas) in enumerate(tqdm(loader)):
            ...
            if args.inversion==True:
                ...
                if use_occ_meta:
                    ...
                    model_kwargs_inv = dict(y=y, cfg_scale=args.cfg_scale)
                # 修复inversion步数范围，确保与扩散配置匹配
                if args.inversion:
                    inversion_steps = range(10, 40)  # 对于ddim50，使用10-40步进行inversion
                else:
                    inversion_steps = range(2, 8)    # 对于ddim10，使用2-8步进行inversion
            ...
            rec_shape=[bz,Tframe,400,400,32]
            if use_vq==False:
                result=my_VQVAE.generate(samples,rec_shape)
            else:
                result=my_VQVAE.generate_vq(samples,rec_shape)
            logit=result["logits"]
            pred = logit.argmax(dim=-1) #  400, 400, 32
            # save npy
            # if i_iter_val < 100:
            if args.save_occ:
                np.save(f"{save_occ_root}/{scene_metas['scene_tokens'][0][0]}_gt_occ.npy", occ_ori.cpu().numpy())
                np.save(f"{save_occ_root}/{scene_metas['scene_tokens'][0][0]}_pred.npy", pred.cpu().numpy())
            ...
            # occ_ori_noT = occ_ori.reshape(-1,400,400,32)
            # pred_noT = pred.reshape(-1,400,400,32)
            # ae_feature_ori = ae_eval.forward_eval(occ_ori) #B*T,16384
            ...
            if vis:
                ...
                for i in range(Tframe):
                    ...
                    ax_ori.set_box_aspect([1,1,0.2])  # 从0.125改为0.2，适应32层高度
                    ax_ori.view_init(elev=90, azim=-90)
                    ax_ori.set_xlim(xmin = -40, xmax = 40)
                    ax_ori.set_ylim(ymin = -40, ymax = 40)
                    ax_ori.set_zlim(zmin = -1, zmax = 5.4)  # 适应32层：32 * 0.2 = 6.4米，从-1到5.4
                ...
                imageio.mimsave(vis_root + f"{iter_num}_{i_iter_val}_{lambda_np}_{args.cfg_scale}_{args.inversion}_hr_mini.mp4",images,'mp4',fps = 4)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--occ-root", dest="occ_root", type=str, default="")
    parser.add_argument("--bev-root", dest="bev_root", type=str, default="")
