...
def main(args):
    ...
    imageset = args.imageset or "data/nuplan_mini_train_clip_infos_dit.pkl"
    bev_path = args.bev_root or "data/nuplan_bev/sample_200"
    gts_path = args.occ_root or "data/occ_quan/nuplan_quantized_200_200_16"

    dataset = Nuplan_Occ_bev_LR_mini(imageset,gts_path,bev_path,bev_ch_use,meta_num=4,Tframe=Tframe,training=True)
    ...
    # 计算总训练步数用于学习率调度
    steps_per_epoch = len(dataset) // (args.dit_batch_size // dist.get_world_size())
    total_steps = args.epochs * steps_per_epoch
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(opt, T_max=total_steps)  # 根据总步数设置学习率衰减
   ...
   # Initial state
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for occ_data,y,occ_meta,pose_meta, scene_metas in tqdm(loader):
            occ_ori = occ_data.to(device)  # 使用输出的occupancy作为训练目标
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lambda-noise-prior", type=float, default=0.15)
