...
def main(local_rank, args):
    ...
    if args.gpus > 1:
        ...
        dist.init_process_group(
            ...
            world_size=hosts * gpus, rank=rank * gpus + local_rank, timeout=timedelta(hours=3))
    ...
    if local_rank == 0:
        ...
        # 初始化 Tensorboard，使用当前时间命名
        current_time = datetime.now().strftime('%Y%m%d_%H%M%S')
        tb_writer = SummaryWriter(log_dir=osp.join(args.work_dir, f'tb_logs_{current_time}'))
    ...
    # with torch.no_grad():
        ...
        for i_iter, (input_occs, target_occs, metas) in enumerate(train_dataset_loader):
            ...
            print(f"==> input_occs shape: {input_occs.shape}, and type: {input_occs.dtype}")
        ...
        with torch.no_grad():
            for i_iter_val, (input_occs, target_occs, metas) in enumerate(val_dataset_loader):
                ...
                if i_iter_val % print_freq == 0 and local_rank == 0:
                    np.save(os.path.join(args.work_dir, f'input_occs_{epoch}.npy'), input_occs.cpu().numpy().astype(np.int8))
                    np.save(os.path.join(args.work_dir, f'pred_occs_{epoch}.npy'), result_dict['sem_pred'].cpu().numpy().astype(np.int8))
