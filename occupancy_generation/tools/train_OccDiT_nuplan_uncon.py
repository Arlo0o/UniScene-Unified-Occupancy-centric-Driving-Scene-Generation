...
def main(args):
    ...
    if rank == 0:
        ...
        ct_str=f"dit_uncon-{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}" 
   ...
   # Initial state
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for i_iter, (occ_ori,y,occ_meta,pose_meta) in enumerate(tqdm(loader, desc=f"Epoch {epoch}", total=len(loader))):
            # if i_iter>100:
            #     break
            ...
            # lambda_z = 0.03 
            noise_prior = torch.randn_like(x) + lambda_np * z_ref # 效果比较弱. 
            # x_ref, 新的 setting. 之前的训练不太对. 需要加上 x_ref的 设置. 
            # - forecasting: 每帧的 bev 都给, 首帧 occ 和 noise concate 一起, 
            #     - occ concate 到 channel维度. 
            # - expansion: 首帧的 occ 和 noise concate 一起
            # expansion 先跑这个: 学习率需要调大一点, 从头跑+fine tune 都一起跑上. 加上填充函数去跑. 重头和 fine tune 的, 可能更需要重头训优先级更高
            # forecasting 的先调 bev 的效果, 长时间的预测看看, 然后再看是否需要 uncond 的 forecasting.  
            # 时序的 occ, 5 帧时序或者重复 5 次的 occ, 看看 vae 的效果, dit 训练的时候还是只用一帧
            # 两台 8 卡机子, 
