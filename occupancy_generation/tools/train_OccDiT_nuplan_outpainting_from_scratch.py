...
def main(args):
    ...
    DiT_cfg={"depth":12, "in_channels":in_ch, "hidden_size":512,"use_label":False, "use_bev_concat":use_bev_concat,"bev_in_ch":1,"bev_out_ch":1,"use_meta":use_occ_meta, "bev_dropout_prob":0.1,"meta_num":meta_num, "Tframe":Tframe}
        ...
        for i_iter, (occ_ori, occ_gt, y, occ_meta, pose_meta) in enumerate(tqdm(loader, desc=f"Epoch {epoch}")):
            ...
            with torch.no_grad():
                ...
                if train_steps % 1000 == 0 and rank == 0:
                    ...
                    # 保存第一个batch的第一帧结果
                    decoded_np = decoded_original.cpu().numpy().astype(np.int8)  # [200, 200, 16]
                    original_np = occ_ori.cpu().numpy().astype(np.int8)  # [200, 200, 16] 原始输入的第一帧
            ...
            if train_steps % 1000 == 0 and rank == 0:
                with torch.no_grad():
                    ...
                    sample_shape = (1, x.shape[1], x.shape[2], x.shape[3], x.shape[4])  # 只取第一个样本但保持多帧
                    ...
                    # 为无条件生成准备kwargs
                    if y is not None:
                        sample_kwargs = {"y": y[:1]}  # 取第一个样本的BEV条件
                    else:
                        sample_kwargs = {"y": None}   # 无条件生成
                    ...
                    try:
                        ...
                        Tframe = generated_latent.shape[2]  # 使用实际的帧数
                        
                        # 不需要扩展，直接使用生成的多帧结果
                        generated_expanded = generated_latent
