    ...
    with torch.no_grad():
        for i_iter_val, (input_occs, target_occs, metas) in enumerate(tqdm(val_dataset_loader)):
            ...
            if i_iter_val % print_freq == 0 and local_rank == 0:
                # np.save(os.path.join(save_occ_dir, f'{metas[0]["scene_token"][0]}_input_occs.npy'), input_occs.cpu().numpy().astype(np.int8))
                # np.save(os.path.join(save_occ_dir, f'{metas[0]["scene_token"][0]}_pred_occs.npy'), result_dict['sem_pred'].cpu().numpy().astype(np.int8)[0])
