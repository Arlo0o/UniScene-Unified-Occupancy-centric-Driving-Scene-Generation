...
from  utils.map_visualizer import visualize_map
...
def draw(
    ...
    # mayavi.mlab.figure(size=(2560, 1440), bgcolor=(1, 1, 1))
    ...
    # plt.savefig(os.path.join(save_dir, f'vis_{timestamp}.png'),bbox_inches='tight', pad_inches=0.0, dpi=300)
...
def main(args):
    ...
    # start_frame = 48
    with torch.no_grad():
        for i_iter_val, (input_occs, target_occs, metas,bevmaps) in enumerate(val_dataset_loader):
            ...
            # bev_map=torch.from_numpy(metas[0]["gt_masks_bev"]).float().unsqueeze(0)
            bev_map=bevmaps[0].float()#torch.from_numpy(bevmaps[0]).float().unsqueeze(0)
            # print(bev_map.shape)
            # import pdb
            # pdb.set_trace() 
            
            map_img_np=visualize_map(bev_map[0])
            map_img=Image.fromarray(map_img_np)
            map_img.save(os.path.join(input_dir, f"{i_iter_val}_map.png"))
            # print(bev_map.shape)
            n_frames=1
            for frame in range(n_frames):
                input_occ = input_occs[:, frame, ...].squeeze().cpu().numpy()
                # print(len(metas))
                # print(metas[:,frame]["gt_masks_bev"])
