...
vis_dir = f"{occ_dir}_vis"
pkl_file = 'data/nuplan_mini_val_clip_infos.pkl'
...
def draw(voxels, voxel_size=0.2, output_path=None):
    # 首先清理任何现有的mayavi实例
    mlab.clf()
    ...
    # 创建新的figure并使用offscreen模式
    figure = mlab.figure(size=(1800, 1800), bgcolor=(1, 1, 1))
...
if __name__ == '__main__':
    ...
    print(f"总共找到 {len(all_tokens)} 个tokens")
    ...
    for i, sample_token in enumerate(all_tokens):
        ...
        # 查找包含该token和GT关键词的文件
        gt_patterns = ['gt_occ', 'input_occs', 'gt']
        pred_patterns = ['pred', 'pred_occs', 'output_occs']
        
        for pattern in gt_patterns:
            potential_gt_path = os.path.join(occ_dir, f'{sample_token}_{pattern}.npy')
            if os.path.exists(potential_gt_path):
                gt_path = potential_gt_path
                break
                
        for pattern in pred_patterns:
            potential_pred_path = os.path.join(occ_dir, f'{sample_token}_{pattern}.npy')
            if os.path.exists(potential_pred_path):
                pred_path = potential_pred_path
                break

        if gt_path is None or pred_path is None:
            print(f'Token {sample_token} files not found: GT={gt_path}, Pred={pred_path}')
            continue
            
        # 打印找到的文件路径
        print(f'  Found GT: {os.path.basename(gt_path)}')
        print(f'  Found Pred: {os.path.basename(pred_path)}')
        ...
        # 构建输出路径
        gt_output_path = os.path.join(vis_root, f'{sample_token}_gt_bev.png')
        pred_output_path = os.path.join(vis_root, f'{sample_token}_pred_bev.png')
        
        # 执行可视化
        draw_nusc_occupancy(
            voxels=gt_voxels, 
            vox_origin=np.array(point_cloud_range[:3]),
            voxel_size=voxel_size,  # 传入完整的体素尺寸数组
