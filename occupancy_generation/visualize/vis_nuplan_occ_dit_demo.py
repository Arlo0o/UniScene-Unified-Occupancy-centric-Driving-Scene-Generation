...
def draw_nusc_occupancy(voxels, vox_origin, voxel_size=[0.2, 0.2, 0.2], grid=None, save_folder=None):
    """绘制NuScenes占用网格"""
    w, h, z = voxels.shape
...
def draw(voxels, voxel_size=0.2, vis_root=None, idx=0):
    ...
    try:
        ...
        figure = mlab.figure(size=(900, 900), bgcolor=(1, 1, 1), fgcolor=(0, 0, 0))
        ...
        # 确保背景为白色
        figure.scene.background = (1.0, 1.0, 1.0)
        figure.scene.foreground = (0.0, 0.0, 0.0)

        # 保存文件
        save_file = f"{occ_dir}_vis"
        os.makedirs(save_file, exist_ok=True)
        filename = str(save_file) + f'/{vis_root[0] if vis_root else "unknown"}_{idx}_bev.png'
        
        print(f"Rendering and saving to: {filename}")
        # 渲染和保存
        mlab.draw()
        mlab.savefig(filename=filename, size=(900, 900))
...
def process_single_file(filepath):
    ...
    try:
        ...
        success = draw_nusc_occupancy(
            ...
            voxel_size=voxel_size,  # 传递完整的voxel_size数组
